...
\section{Motivation:}
Many new distributed computing technologies are now available to support performant, scalable, cost-efficient genomic data analysis. However,
current genomics data formats and processing pipelines were developed prior to many of these technologies and so can't take advantage of their unique capabilities. Through the careful
design of new file formats, we can exploit current distributed computing technologies and ensure genomic data and tools  can readily be optimized for future computing advances.
...
The improvement of next generation sequencing~(NGS) methods has made data processing a bottleneck~\citep{mcpherson09}, particularly for those working in time-sensitive clinical settings or with many thousands of samples. Numerous other industries, particularly in the web-space, have faced similar challenges and as result a number of new technologies have been developed to enable performant, scalable, cost-effective distributed computing~\citep{barroso13}. Key genomic file file formats, specifically Sequence/Binary Alignment/Map~(SAM/BAM) and Variant Call Format~(VCF), were designed before many of these distributed computing technologies were widely available and so are not designed to take advantage of their specific capabilities~\citep{li09,niemenmaa12}.

The design choices in BAM and VCF made to optimize for the tools and computing systems available at the time, such as a row/record-centric design and a single header, also limit scaling to larger distributed computing systems that do not provide shared storage~\citet{niemenmaa12}. We choose to reimplement the key genomic file formats to unlock the advantages of modern distributed computing platforms, and to enable easy adoption
...
ADAM is fully open source under the Apache 2 license, and is implemented on top of Avro\footnote{\url{http://avro.apache.org}} and Parquet\footnote{\url{http://www.parquet.io}} for data storage. Our reference pipeline
...
% NOTE: This is redudant since almost the same text appears earlier except
% the Thrift and Protobuf references, which do not add much information/context
% for this paper (IMHO, Andre).
%
%The ADAM schema is represented using Apache Avro, an open-source, cross-platform data serialization framework similar
%to Apache Thrift and Google's Protocol Buffers~\citep{thrift, protobuf}.
Apache Avro was chosen for the ADAM interchange data format for several reasons:
...
an API, as well as an API for enriched datatypes. These APIs are similar to those provided by SAMtools~\citep{li09} and Picard\footnote{\url{http://picard.sourceforge.org}}.
...
framework. We have already explained the reason for using Avro, we now give the rationale for using Spark and Parquet.
...
Although it is compatible with the Hadoop~\citep{white09} ecosystem, Spark differs from conventional implementations of the Map-Reduce framework
...
The data representation for the ADAM format is described using the open source Apache Avro data serialization system. The Avro system also provides a human
...
SAMtools~\citep{li09} and for Java through Picard. Additionally, there are known incompatibilities between these different implementations---by using Avro,
...
% The 2nd sentence needs a lot of work, it is too vague
...
\begin{itemize}
% What does "complex" mean here, I think you want to say data can be annotated in a natural representation, i.e. records don't need to be flattened to strings.
...
However, tools which require SAMs/BAMs in coordinate-sorted order often rely on the content \emph{and} ordering of the sequence dictionaries between the files to be preserved\footnote{See, for example, \url{http://www.broadinstitute.org/gatk/guide/article?id=1204}}, and fail when this requirement is not met.  
Separate command-line tools are required to re-order the sequence dictionaries of multiple files into a common order.
...
% This first sentence is content free - what "patterns"?
...
\item System setup can be simplified through the use of systems like \textit{Docker}\footnote{\url{https://www.docker.io/}} for distributing consistent application images.\todo{Is mentioning Docker here really useful/relevant?}
...
% I think this should all be moved to introduction, it provides a great vehicle for setting the theme of adapting the semantics of these formats with a new syntax.
...
For example, ADAM data can easily be accessed using relational databases like Shark~\citep{xin13shark} and Impala\footnote{\url{http://www.cloudera.com/content/cloudera/en/products/cdh/impala.html}}. Additionally, ADAM can be used as the base
...
% What is the process that should be followed? Don't leave hangers...
...
% I think this is getting ahead of yourself, I would delete
...
% what is SQL querying, that is a "term" without a definition, do you mean query with SQL
