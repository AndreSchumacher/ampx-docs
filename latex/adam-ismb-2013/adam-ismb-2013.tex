...
desirable to be able to process this data using very large clusters or via cloud computing. We introduce a new set of language-independent file formats that are designed for
cluster and cloud computing as a potential successor to the BAM and VCF standards.

\section{Results:} We propose ADAM, which is a set of file formats and command line tools for processing genome data on clusters and in the cloud. On a high coverage~(60$\times$) 250GB
NA12878 BAM file, with ADAM we are able to perform Sorting and Duplicate Marking in under 50 minutes on a 100 node
cloud-based cluster~(a 50$\times$ performance improvement). On a single node, our tools performs twice as fast as Picard and SAMtools. ADAM files are up to 25\%
...
very large datasets for genome wide association studies~(GWAS, see~\citet{wang05})\todo{This sentence seems quite complicated and ''as'' is repeated. Could it be simplified?}. To address this issue, it is desirable to be able to compute on genomic data across
...
The scalability limitations are inherent to the file formats, as we discuss in more detail in \S\ref{sec:approach}.
We choose to rethink file formats for genomics
...
\todo{Instead of formulating these as (research?) questions, which is a bit artificial, it would seem stronger to formulate them as design goals.}
...
Apache Spark is an in-memory, distributed Map-Reduce framework that was introduced in \citet{zaharia10} and further described in \citet{zaharia12}.\todo{Do we
...
to the ADAM repository.\todo{Note: there should be some notion of how users deal with changes in format (backward compatibility, default values?)!}
