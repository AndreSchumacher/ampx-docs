...
rewriting these applications to make use of modern in-memory MapReduce frameworks like Apache Spark~\cite{zaharia10}, these transformations can now be completed in FIXME\footnote{FIXME} hours.
...
BAM) from the 1,000 Genomes Project.
...
\begin{table}[h]
\caption{Time to place a file on HDFS}
\label{tab:time-flagstat}
\begin{center}
\begin{tabular}{| l | c | c |}
\hline
\bf File Size  & \bf Cluster Size \\
\hline
TODO & TODO \\
TODO & TODO \\
\hline
...
TODO: provide some numbers on HDFS load times (including the speedup found using distcp -- e.g. a 250GB on
S3 can be written to HDFS in just over 20 minutes -- ganglia reported sustained ~450Mb/s). Note: we did not load
Hadoop-native for these tests which would have helped with performance.
...
TODO: talk about conversion from BAM to ADAM

TODO: Explain an ADAMRecord
...
\hline
Samtools 0.1.19 & 25m24s (todo: verify) \\ 
ADAM 0.5.0 & 0m46s \\
\hline
\end{tabular}
\end{center}
...
\begin{scriptsize}
\begin{verbatim}
 1: def adamSortReadsByReferencePosition(): RDD[ADAMRecord] = {
 2:  rdd.map(p => {
 3:    val referencePos = ReferencePosition(p) match {
 4:      case None =>
 5:        // Move unmapped reads to the end
 6:        ReferencePosition(Int.MaxValue, Long.MaxValue)
 7:      case Some(pos) => pos
 8:    }
 9:    (referencePos, p)
10:  }).sortByKey().map(p => p._2)
11: }
\end{verbatim}
\end{scriptsize}
...
\hline
Picard Tools 1.102 & TODO & 13h (todo: verify this) \\
...
command. The following ADAM code, reformatted for this report, expresses the algorithm
succinctly in 42 lines of Scala code, e.g.

\begin{scriptsize}
\begin{verbatim}
 1:for (((leftPos, library), readsByLeftPos) <- 
 2:    rdd.adamSingleReadBuckets()
 3:       .keyBy(ReferencePositionPair(_))
 4:       .groupBy(leftPositionAndLibrary);
 5:
 6:buckets <- {
 7:
 8:leftPos match {
 9: // These are all unmapped reads. 
10: // There is no way to determine if they are duplicates
11: case None =>
12:    markReads(readsByLeftPos.unzip._2, areDups = false)
13: // These reads have their left position mapped
14: case Some(leftPosWithOrientation) =>
15:   // Group the reads by their right position
16:   val readsByRightPos = readsByLeftPos.groupBy(rightPosition)
17:   // Find any reads with no right position
18:   val fragments = readsByRightPos.get(None)
19:   // Check if we have any pairs (reads with a right position)
20:   val hasPairs = readsByRightPos.keys.exists(_.isDefined)
21:   if (hasPairs) {
22:     // Since we have pairs, mark all fragments as duplicates
23:     val processedFrags = if (fragments.isDefined) {
24:       markReads(fragments.get.unzip._2, areDups = true)
25:     } else {
26:       Seq.empty
27:     }
28:     val processedPairs = 
29:         for (buckets <- (readsByRightPos - None).values;
30:             processedPair <- 
31:                    scoreAndMarkReads(buckets.unzip._2)) 
32:        yield processedPair
33:     processedPairs ++ processedFrags
34:   } else if (fragments.isDefined) {
35:     // No pairs. Score the fragments.
36:     scoreAndMarkReads(fragments.get.unzip._2)
37:   } else {
38:     Seq.empty
39:   }
40:}
41:};
42:read <- buckets.allReads) yield read
\end{verbatim}
\end{scriptsize}
...
\bf Software & \bf EC2 profile & \bf Wall clock time \\
\hline
Samtools rmdup & TODO? & TODO? \\
Picard MarkDuplicates 1.102 & TODO & 13h (todo: verify this) \\
...
TODO: talk about differences between ADAM and Picard, e.g.
ADAM does not require sorted input, 

\subsubsection{Mark Duplicates and Sort In-Memory}
\label{sec:mark-duplicates-sort}
...
\bf Software & \bf EC2 profile & \bf Wall clock time \\
\hline
Picard Tools 1.102 & TODO & 26h (todo: verify this) \\
