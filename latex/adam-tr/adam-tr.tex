...
APIs~(\S\ref{sec:data-format-and-api}) and programing model~(\S\ref{sec:in-memory-programming-model}). Finally,
we review the performance and compression gains we achieve~(\S\ref{sec:performance}), and outline future enhancements
to ADAM that we are working on~(\S\ref{sec:future-work}).
...
The ADAM source code is available at Github at \url{http://www.github.com/massie/adam}. Additionally, ADAM is
deployed through Maven with the following dependency:
