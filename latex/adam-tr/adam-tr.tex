...
We can prove that this process requires no additional reads: intuitively, the initial columns read would need to be read if they were to be
used in the filter later. Formally, if $R$ is the set of records to read, $Proj$ is the projection we are using, and $size(i,j)$ returns the amount
